...
REM Work out name of dir containing this batch file (assumes filename is broker.bat which is 10 chars long)
set this=%~f0
set dir=%this:~0,-11%
...
set jsse_jars_dir=%IMQ_EXTERNAL%\jsse\lib

set _classes=%_IMQ_HOME%\..\..\share\opt\classes;%jsse_jars_dir%\jsse.jar;%jsse_jars_dir%\jnet.jar;%jsse_jars_dir%\jcert.jar;

%JAVA_HOME%\bin\java -cp %_classes% %JVM_ARGS% com.sun.messaging.jmq.jmsserver.Broker %*
...
:restart
REM # errorlevel is 255 or greater. Make sure it is not greater
if ERRORLEVEL 256 goto end
REM # Sleep so if we get into this state accidentally we don't peg system 
sleep 1
goto resume
