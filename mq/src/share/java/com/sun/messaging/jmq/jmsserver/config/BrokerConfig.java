...
// Time zone to use for logging timestamps. If not specified use the
// system time zone. For example to use GMT:
// imq.log.timezone=GMT
// The time zone identifiers supported are the same as those used by
// java.util.TimeZone.getTimeZone(). Some examples:
// "GMT", "America/Los_Angeles", "Europe/Rome", "Asia/Tokyo"
imq.log.timezone=
...
// the file age (seconds). -1 means don't rollover based on that criteria.
imq.log.file.rolloverbytes=-1
...
imq.debug.com.sun.messaging.jmq.jmsserver.persist.Store=false
...
{
    private static final String IMQ = Globals.IMQ;
	...
	+ IMQ + ".persist.file.message.vrfile.growth_factor=50\n"
        ...
        + ShareConfigChangeStore.STORE_TYPE_PROP+"="+ShareConfigChangeStore.DEFAULT_STORE_TYPE+"\n"
        + ShareConfigChangeStore.CREATE_STORE_PROP+"=true\n"
        + IMQ + ".memory.levels=green,yellow,orange,red\n"
        + IMQ + ".memory.hysteresis=1024\n"
        + IMQ + ".memory.overhead=10240\n"
        + IMQ + ".memory.gcdelta=1024\n"
     ...
     */
    private static final String JMQ_prop_loc = Globals.JMQ_LIB_HOME + File.separator +
                                               Globals.JMQ_BROKER_PROP_LOC;
    ...
    public BrokerConfig(String configname, Properties params, boolean resetProp, Properties saveProps) throws BrokerException {
        ...
        logger.log(Logger.DEBUG, BrokerResources.I_JMQ_HOME,Globals.JMQ_HOME);
